...
WITH seating_types AS (
   SELECT DISTINCT
       ...
       -- Handle sidewalk approval logic
       CASE 
           WHEN approved_for_sidewalk_seating IN ('yes', 'Yes', 'Y') THEN TRUE 
           ELSE FALSE 
       END AS approved_for_sidewalk,

       -- Handle roadway approval logic
       CASE 
           WHEN approved_for_roadway_seating IN ('yes', 'Yes', 'Y') THEN TRUE 
           ELSE FALSE 
